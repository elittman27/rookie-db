...
public class TestLockContext {
    ...
    @Test
    @Category(PublicTests.class)
    public void testSIXPromote() {
        TransactionContext t1 = transactions[1];
        LockContext page2LockContext = tableLockContext.childContext("page2");

        dbLockContext.acquire(t1, LockType.IX); // Will be SIX
        tableLockContext.acquire(t1, LockType.IX); // Will be IX
        pageLockContext.acquire(t1, LockType.S); // Will be NL
        page2LockContext.acquire(t1, LockType.X); // Will be X


        assertTrue(dbLockContext.getNumChildren(t1) == 1); // db context has one child
        assertTrue(tableLockContext.getNumChildren(t1) == 2);
        assertTrue(pageLockContext.getNumChildren(t1) == 0); 
        assertTrue(page2LockContext.getNumChildren(t1) == 0); 

        dbLockContext.promote(t1, LockType.SIX); // Releases the table lock context's S lock

        assertTrue(dbLockContext.getNumChildren(t1) == 1); // db context has one child
        assertTrue(tableLockContext.getNumChildren(t1) == 1); // db context has one child
        assertTrue(pageLockContext.getNumChildren(t1) == 0); // db context has one child
        assertTrue(page2LockContext.getNumChildren(t1) == 0); // db context has one child
        ...
        assertTrue(TestLockManager.holds(lockManager, t1, page2LockContext.getResourceName(), LockType.X));
    }
